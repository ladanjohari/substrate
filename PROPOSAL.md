...
*Clean version, updated 2 September 2026. Replaces the draft outline in the shared doc.*
...
|---|---|---|
| Substrate (data store + CLI) | goals, tasks, exit criteria, blocking edges, the event log | **shipped 2 Sep**, public repo with a README |
| Agents | a decomposer that turns a goal into a tree, workers that take tasks, a checker that marks criteria met with evidence | working, one agent at a time |
...
|---|---|---|
| 1. Substrate CLI and data store | Public GitHub repo with a README. A stranger can install it and run all seven commands: create a task with exit criteria, change a criterion's state, edit a description, block one task on another, delete a task, recalculate the critical path, show the tree. | **Done, Wed 2 Sep** |
| 1A. Claude skill | The skill file ships in the same repo and is documented in the README. | **Done, Wed 2 Sep** |
...
## The seven CLI commands (piece 1, all shipped)

| Command | Does |
|---|---|
| `substrate add` | create a task with exit criteria |
| `substrate meet` / `fail` | change the state of an exit criterion, with evidence |
| `substrate edit` | change a task's description |
| `substrate block` / `unblock` | block one task on another, or undo it |
| `substrate remove` | delete a task, or a goal and its tasks |
| `substrate path` | recalculate the critical path |
| `substrate tree` | show the state of the tree |

Two more were added because the seven alone do not get a person from an idea to
running work:

| Command | Does |
|---|---|
| `substrate decompose` | one sentence in, a proposed task tree with exit criteria out |
| `substrate approve` | releases a goal to the agents; nothing runs before it |
...
The draft says "React-based". A menu bar app needs a shell, and React cannot sit
in the menu bar on its own. Two ways to get there:
...
## Known gap, to close in piece 3

Today one agent works one task at a time. Several agents in parallel, subagents
underneath them, and a view of which agent is on which task are all part of the
menu bar app, not of the CLI that shipped.
